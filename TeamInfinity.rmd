...
fpath <- "E:/NEU/Courses/CS5200/Practicum/Practicum 2/"
...
CREATE TABLE JournalDim 
  AS SELECT ISSN, IssnType, Title, ISOAbbreviation FROM Journal;
```

```{sql connection ="dbcon"}
CREATE TABLE AuthorDim 
  AS SELECT AuthorId, LastName, ForeName, Initials, Affiliation FROM Author;
```

```{sql connection ="dbcon"}
CREATE TABLE HistoryDim 
  AS SELECT HistoryID, PubStatus, PubDate FROM History;
```
